...
書き手（自書き義務）は Write を持つ `tdd-strict-coder` / `implementation-coder` / `testability-architect` とメインに限定。レビュアー 3 者は「読む義務 + 報告に含める」に統一し、正準ブロック `generators/agent-blocks/trial-log-reviewer.md` にこの契約を明文化した。
