...
レビュー着手前に、同じリポジトリの `docs/trial-log/` を確認する（`ls` で一覧し、今回のレビュー対象に関連する試行内容のファイルがあれば読む）。
- **目的**: 既に棄却された案が「改善提案」として再提示されていないか確認する。棄却理由の妥当性そのものも検証する。
- 該当ファイルがなければスキップ（詳細は `rules/30-documentation-management.md`「読む義務」）。
- 棄却理由に異議がある、または新しい状況で再試行する価値がある場合は、その旨を報告に含める。
