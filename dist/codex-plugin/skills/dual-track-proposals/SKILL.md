...
2. **Parallel dispatch** (one message, two Agent calls):
   - `外部ベンダー相当ツール（利用可能なら）` (クロスベンダー最上位ティア導入時) — state read-only explicitly in the task text (propose only, do not edit) so the forwarder does not add `--write`. Ask for design approach + change outline (files touched, pseudo-diff level).
